...
?>
<?php
$remove_friend=$_POST['delete'];
$delete_friend_id=$_POST['delete_friend_id'];
if(isset($remove_friend)){
	$remove_relation=mysql_query("delete from friends where user_id='$user' and follows_id='$delete_friend_id'");
	if(!$remove_relation) die ("Remove relation error: " . mysql_error());
	$no_friend=mysql_query("update friends set friend=0 where user_id='$delete_friend_id' and follows_id='$user'");
	if(!$no_friend) die ("Remove friend error: " . mysql_error());
	header("Location:my_friends_page.php?id=$user");
}
?>
<!DOCTYPE html>
<html>
			...
			<?php
					...
					for ($j =0; $j <$rs ; ++$j){	
						...
						echo	"</a></div>
								...
								<form method='post' action='my_friends_page.php'>
								<div class='delete'>
									<abbr title='Delete from friends'>
									<label>
									<i class='fa fa-close'></i>
										<input type='submit' name='delete' id='hidden-delete' value='.'>
									</label>
										<input type='text' name='delete_friend_id' value='$id_user' id='none'>
									</abbr>
								</div>
								</form>
							</div>";
