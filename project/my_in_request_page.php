...
?>
<?php
$add_friend=$_POST['accept_request'];
$add_friend_id=$_POST['accept_request_id'];
if(isset($add_friend)){
	$add_relation=mysql_query("insert into friends (user_id, follows_id) values ('$user', '$add_friend_id')");
	if(!$add_relation) die ("Remove relation error: " . mysql_error());
	$my_friend=mysql_query("update friends set friend=1 where user_id='$user' and follows_id='$add_friend_id'");
	if(!$my_friend) die ("My friend error: " . mysql_error());
	$p_friend=mysql_query("update friends set friend=1 where user_id='$add_friend_id' and follows_id='$user'");
	if(!$p_friend) die ("P_friend error: " . mysql_error());
	header("Location:my_in_request_page.php?id=$user");
}
?>
<!DOCTYPE html>
<html>
...
function topFunction() {
  ...
  document.documentElement.scrollTop = 0;
	...
	<section class="container">
		...
		<div class="second">
			<?php
					$have_friend=mysql_query("select * from friends where follows_id='$user' and friend=0");
					...
					for ($j =0; $j <$rs ; ++$j){	
						...
						$user_n=mysql_query("select user_id, name, surname, image, city, login from users where user_id=$r[0]");
						...
						echo	"</a></div>
								<div class='friend-info'>
									...
									<form method='post'>
									<input type='submit' name='accept_request' id='btn' value='Add to friends'>
									<input type='text' name='accept_request_id' value='$id_user' id='none'>
									</form>
								</div>
