...
function topFunction() {
  ...
  document.documentElement.scrollTop = 0;
	...
	<section class="container">
		...
		<div class="second">
				<center><font style="font-weight: bold; font-size: 18px;">Liked Posts</font></center>
					<?php
					$friend=mysql_query("select post_id from post_like where user_id='$user'");
					...
					$results=mysql_query("select * from posts where post_id in ($comma_separated) order by post_id desc");
					...
					for ($i = 1 ; $i <=$row_post ; ++$i){	
						...
						if(!empty($post_video_name)){
						 ...
						 echo "</div>
                                ...
                                ".make_like($user, $postId)." 
                                ".make_posts($postId)."
							";
                            if($author_id!=$user){
								echo			
									"<abbr title='Share post' style='margin-left:20px;'>
									<label>
