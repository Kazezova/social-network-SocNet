...
if(isset($delete_photo)){
	$delete_tags=mysql_query("delete from tags where user_id='$user' and photo_id='$photo_id'");
	if(!$delete_tags) die ("Remove error: " . mysql_error());
	$delete_likes=mysql_query("delete from photo_like where photo_id='$photo_id'");
	$delete_comments=mysql_query("delete from photo_comment where photo_id='$photo_id'");
	$delete=mysql_query("delete from photos where user_id='$user' and photo_id='$delete_photo_id'");
	if(!$delete) die ("Remove error: " . mysql_error());
	header("Location:my_photos_page.php?id=$user");
}
?>
<?php
$make_profile_photo=$_POST['make_profile_photo'];
$make_profile_photo_id=addslashes(file_get_contents("data:image/jpeg;base64, '$image'"));
if(isset($make_profile_photo)){
	$make=mysql_query("update users set image='$make_profile_photo_id' where user_id='$user'");
	if(!$make) die ("Make error: " . mysql_error());
	header("Location:my_photos_page.php?id=$user");
}
?>
<?php
$new_photo_description=$_POST['photo_description'];
if(isset($new_photo_description)){
	$new_description=mysql_query("update photos set photo_description='$new_photo_description' where user_id='$user' and photo_id='$photo_id'");
	if(!$new_description) die ("Make error: " . mysql_error());
	header("Location:my_photos_page_detail.php?photo_id=$photo_id&profile_id=$user");
}
?>
<?php
$tag_person=$_POST['tag_person'];
$tag_person_id=$_POST['tag_person_id'];
if(isset($tag_person)){
	$insert_tag=mysql_query("insert into tags (user_id, photo_id, person_id) values ('$user','$photo_id', '$tag_person_id')");
	if(!$insert_tag) die ("Insert tag error: " . mysql_error());
	header("Location:my_photos_page_detail.php?photo_id=$photo_id&profile_id=$user");
}
?>
<?php
$unmark=$_POST['unmark'];
$unmark_id=$_POST['unmark_id'];
if(isset($unmark)){
	$unmark_person=mysql_query("delete from tags where user_id='$user' and photo_id='$photo_id' and person_id='$unmark_id'");
	if(!$unmark_person) die ("Remove error: " . mysql_error());
	header("Location:my_photos_page_detail.php?photo_id=$photo_id&profile_id=$user");
}
?>
<?php
				...
				<?php
					...
					for ($j =0; $j <$rs ; ++$j){	
						...
						<div class='each-photo'>
							<a href='my_photos_page_detail.php?photo_id=$image_id&profile_id=$user'><img src='data:image/jpeg;base64,$image_user' id='myImg '></a>
						</div>";
					}
				?>
			</div>
			    ...
			    <div class='interactive'>
				    	<div class="my-interactive">
				    		<div class='delete-photo'>
				    			<form method="post">
								<input type='submit' name='delete_photo' id='interactive-button' value='Delete photo'>
								<input type='text' name='delete_photo_id' value='<?=$photo_id?>' id='none'>
								</form>
							</div>
							
							<div class="profile-photo">
								<form method="post">
								<input type="submit" name="make_profile_photo" id="interactive-button" value="Make profile photo">
								</form>
							</div>
							
							<div class='tag'>
								<button id='interactive-button' onclick="myTagFunction()" class="tag-btn">Tag a person</button>
								<form method='post' action=''>
								<?php
								$tag_friends=mysql_query("select person_id from tags where user_id='$user' and photo_id='$photo_id'");
								...
								if(empty($comma_separated)){
									$have_friend=mysql_query("select * from friends where user_id='$user' and friend=1");
								}
								else{
									$have_friend=mysql_query("select * from friends where user_id='$user' and friend=1 and follows_id not in ($comma_separated)");
								}
								if (!$have_friend) die ("Select * error: " . mysql_error());
								
								$friend_num=mysql_num_rows($have_friend);
								$new_array=array();
								if($check!='true'){
									array_push($new_array, $user);
								}
								for($q=1; $q<=$friend_num; ++$q){
									$tag_row=mysql_fetch_row($have_friend);
									$f_id=$tag_row[1];
									array_push($new_array, $f_id);
								}
								$comma=implode(",", $new_array);
								$mark=mysql_query("select user_id, name, surname, image from users where user_id in ($comma)");
								if($mark){
								$rs=mysql_num_rows($mark);
							}
								?>
								<div id="myTagDropdown" class="tag-dropdown-content">
								<?php

									for ($j =0; $j <$rs ; ++$j){	
									// $r = mysql_fetch_row($mark);
									// if (!$user_n) die ("Select name error: " . mysql_error());
									$table_user = mysql_fetch_assoc($mark);
								...
								echo "<div class='each-row'>";
								if(!empty($image_user)){?>
										<img src='data:image/jpeg;base64, <?=$image_user ?>' class='round-image' width='40px' height='40px' style='object-fit: cover;'>
								<?php }
								else{
									?>	<img src='account_1.png' class='round-image' width='40px'>
									<?php
									}
								echo "<form method='post'><input type='submit' name='tag_person' value='$name_user $surname_user' id='each-row-input'>
									<input type='text' name='tag_person_id' value='$id_user' id='none'></form>";
								echo "</div>";
								}
								?>
								...
								function myTagFunction() {
								  document.getElementById("myTagDropdown").classList.toggle("show");
								};
								$(document).on('click','.interactive', function(event) {
								  if (!event.target.matches('.tag-btn')) {
								    var dropdowns = document.getElementsByClassName("tag-dropdown-content");
								    var i;
								    for (i = 0; i < dropdowns.length; i++) {
								      var openDropdown = dropdowns[i];
								      if (openDropdown.classList.contains('show')) {
			        ...
			        <div id="caption">
				            ...
				            <div class="like">
				              	<font style="display: flex;">
				              		<?php
										...
										$output = '<abbr title="Unlike"><label><form method="post"><i class="fa fa-heart" style="color:red;"><input type="submit" name="action_unlike" value="0" id="hidden-unmark"></form></i></label></abbr>';
										}
										else{
										$output = '<abbr title="Like"><label><form method="post"><i class="fa fa-heart-o" style="color:red;"><input type="submit" name="action_like" value="1" id="hidden-unmark"></form></i></label></abbr>';
										}
										return $output;
				 						}
				        ...
				        <div class="photo-info">
				        			...
				        			for($a=1; $a<=$select_tag_num; ++$a){
										...
										echo "$name_u $surname_u</a>
												...
												<div class='delete'>
													...
													<label>
													<i class='fa fa-close'>
													<input type='submit' name='unmark' id='hidden-unmark' value='.'>
													</i>
													</label>
														<input type='text' name='unmark_id' value='$id_u' id='none'>
													</abbr>
												</div>
												</form>
												</div>
				            ...
				            <div class="author-comment">
				            	<textarea class="submit_on_enter" type="text" name="photo_description" placeholder="Add description" id="new-description"><?=$photo_description?></textarea>
				           	</div>
				            </form>
				        </div>
